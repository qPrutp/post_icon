...
function update_post_icon_table($status) {
	...
	echo $status['posts_id'] . $status['icon'] . $status['siting_icon'];
}
...
function add_icon_title($title) {
	global $wpdb;
	$publish_post_list_id = $wpdb->get_col('SELECT a.id_post, a.icon_name, a.position_icon FROM wp_post_icon a, wp_posts b WHERE a.id_post = b.ID AND b.post_status = "publish";');
...
function true_option_page(){
	...
	// foreach ($publish_post_list_id as $key => $value) {
	// 	echo "<pre>";
	// 	echo $publish_post_list_id[$key]->id_post;
	// 	echo "<pre>";
	// 	$new_title = get_the_title($publish_post_list_id[$key]->id_post);
	// 	if($publish_post_list_id[$key]->id_post == 5) {
	// 		$new_title = $new_title . "<span class='dashicons dashicons-lock'></span>";
	// 	}
	// 	echo $new_title;

	// 	$post_update = array(
	// 		'ID'         => $publish_post_list_id[$key]->id_post,
	...
	switch ( $type ) {
		case 'posts_list':
			...
			foreach($content as $v=>$l){
				...
				echo "<option value='$content[$v]' $selected>$l</option>";
			}
			echo ($desc != '') ? $desc : "";
			echo "</select>";
		break;
...
function add_icon_title() {
	global $wpdb;
	$publish_post_list_id = $wpdb->get_col('SELECT a.id_post, a.icon_name, a.position_icon FROM wp_post_icon a, wp_posts b WHERE a.id_post = b.ID AND b.post_status = "publish";');
	echo "add_icon_title is working!";
}
if($status_plugin == 1) {
	add_action('add_icon', 'add_icon_title');
}
...
function posts_icon_settings() {
	global $wpdb;
	$post_list = $wpdb->get_col('SELECT post_title FROM `wp_posts` WHERE post_status = "inherit" AND comment_status = "closed" AND ping_status = "closed"');
	$post_list_id = $wpdb->get_col('SELECT ID FROM `wp_posts` WHERE post_status = "inherit" AND comment_status = "closed" AND ping_status = "closed"');
	foreach ($post_list_id as $key => $value) {
		$post_list_id[$key] = $value - 1;
	}
	global $true_page;
	global $dashicons;
	// Присваиваем функцию валидации ( true_validate_settings() ). Вы найдете её ниже
